...
import { geometry } from "../src/display-geometry.js";
...
function refresh(page: InspectPage, horizontal = false): InspectPage {
  for (const i of page.textItems) i.displayGeometry = geometry({ x: i.displayX, y: i.displayY },
    { x: i.displayX + (horizontal ? i.width : 0), y: i.displayY + (horizontal ? 0 : i.height) },
    horizontal ? { x: 0, y: -i.fontSize } : { x: i.fontSize, y: 0 });
  return page;
}
...
function fixture(horizontal = false): InspectPage {
  ...
  return refresh({ page: 1, width: 600, height: 800, rotation: 0, userUnit: 1,
    view: [0, 0, 600, 800], textItemCount: textItems.length, imagePaintOps: 0, textItems }, horizontal);
}
...
for (const horizontal of [false, true]) {
  ...
  test(`associates ${orientation} ruby candidates with source run indexes without mutation`, () => {
    ...
    const [candidate] = associateRubyCandidates(refresh(page, horizontal), orientation, 14);
...
test("retains ambiguous overlapping base runs instead of choosing the first", () => {
  ...
  const [candidate] = associateRubyCandidates(refresh(page), "vertical", 14);
...
test("nearby small text without inline overlap is not ruby-associated", () => {
  ...
  assert.equal(associateRubyCandidates(refresh(page), "vertical", 14)[0]?.reason, "no-nearby-run");
});
...
test("invalid body size and body-sized text do not create ruby candidates", () => {
  ...
  assert.deepEqual(associateRubyCandidates(refresh(page), "vertical", 14), []);
});
...
test("scaled and translated geometry gives the same association", () => {
  ...
  page.page = 99;
  assert.deepEqual(associateRubyCandidates(refresh(page), "vertical", 28)[0]?.baseItemIndexes, [1]);
});
