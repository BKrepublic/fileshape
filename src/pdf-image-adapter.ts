...
// Pinned PDF.js 6.3.289 constructPath uses compact DrawOPS values in its
// Float32 path buffer, not the public OPS values.
...
function bounds(points: XY[]): DisplayRect {
...
function compactRectangleCorners(path: number[]): XY[] | undefined {
  // A PDF `re` rectangle is normalized by PDF.js into
  // moveTo + lineTo + lineTo + lineTo + closePath.
  if (path.length === 13 && path[0] === DRAW_MOVE_TO && path[3] === DRAW_LINE_TO &&
      path[6] === DRAW_LINE_TO && path[9] === DRAW_LINE_TO && path[12] === DRAW_CLOSE_PATH) {
    return [
      { x: path[1]!, y: path[2]! },
      { x: path[4]!, y: path[5]! },
      { x: path[7]!, y: path[8]! },
...
function transformedRectangleBounds(path: number[], displayMatrix: number[]): DisplayRect | undefined {
  const sourceCorners = compactRectangleCorners(path);
  if (!sourceCorners) return undefined;
  const corners = sourceCorners.map((entry) => point(displayMatrix, entry.x, entry.y));
  const tolerance = 1e-7;
  const horizontal = (a: XY, b: XY) => Math.abs(a.y - b.y) <= tolerance;
  const vertical = (a: XY, b: XY) => Math.abs(a.x - b.x) <= tolerance;
  const [p0, p1, p2, p3] = corners;
  const axisAligned = p0 && p1 && p2 && p3 &&
  ...
  const result = bounds(corners);
  if (result.left === result.right || result.top === result.bottom) return undefined;
  return result;
}
