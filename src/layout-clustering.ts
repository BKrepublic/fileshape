...
export type VerticalTextLayoutMode = "glyph" | "run";
...
export function singleCharItemRatio(items: InspectTextItem[]): number {
  if (items.length === 0) return 0;
  return items.filter((item) => charCount(item.text) === 1).length / items.length;
}

export function glyphSequenceRatios(items: InspectTextItem[]): {
  vertical: number;
  horizontal: number;
} {
...
/**
 * Select the shared vertical reconstruction mode from representation evidence.
 * Rounding mirrors PageFlowResult metrics so flow and physical layout cannot
 * disagree solely because one stage compared raw ratios and another retained
 * compact four-decimal evidence.
 */
export function verticalTextLayoutMode(items: InspectTextItem[]): VerticalTextLayoutMode {
  const sequence = glyphSequenceRatios(items);
  const singleCharRatio = round(singleCharItemRatio(items), 4);
  const verticalSequenceRatio = round(sequence.vertical, 4);
  return singleCharRatio >= 0.7 && verticalSequenceRatio >= 0.6 ? "glyph" : "run";
}
