...
function pngBytes(kind: PdfImagePixelKind, width: number, height: number, source: Uint8Array): Uint8Array {
...
export function extractPdfImageResource(sourceResourceId: string, value: unknown): PdfImageResourceResult {
  ...
  const bytes = pngBytes(format, image.width, image.height, copied);
...
export function resolvePdfImageResource(store: PdfObjects, sourceResourceId: string): PdfImageResourceResult {
  try {
    return extractPdfImageResource(sourceResourceId, store.get(sourceResourceId));
  } catch (error) {
    return {
      sourceResourceId,
      status: "unsupported",
