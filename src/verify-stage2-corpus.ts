...
async function analyzePdf(filePath: string): Promise<FileSummary> {
  ...
  for (const page of inspection.pages) {
    if (!hasVisibleText(page)) continue;
    textPages += 1;

    const flow = reconstructPageFlow(page);
    if (flow.orientation === "unknown") {
      issues.push({
        file: inspection.file,
        page: page.page,
        kind: "unknown-orientation",
        detail: "text-bearing page has unknown writing orientation",
      });
      snapshots.push({ page: page.page, orientation: flow.orientation, semanticText: "" });
      continue;
    }
    ...
    if (physical.units.length === 0) {
      issues.push({
        ...
        detail: "text-bearing page produced no physical text units",
      });
      snapshots.push({ page: page.page, orientation: flow.orientation, semanticText: "" });
      continue;
    }
...
function printFail(
  ...
  if (summaries.length > 0) {
    ...
    for (const summary of summaries) {
      console.log(
        `${summary.file}: pages=${summary.pageCount}, text=${summary.textPages}, semantic=${summary.semanticPages}, issues=${summary.issues.length}`,
      );
    }
  }
